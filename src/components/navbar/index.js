import { useEffect, useState } from "react";
import { NavLink, useNavigate } from "react-router-dom";
import { useSelector, useDispatch } from 'react-redux';
import { useMediaQuery } from 'react-responsive';
// import { useComponentVisible } from "hooks/useVisible";
import { updateUser } from "features/authSlice";
import './index.css';
import { useTheme } from "hooks/useTheme";
import { BiSun } from 'react-icons/bi';
import { BsFillMoonFill } from 'react-icons/bs';
import { MobileNavigation } from "./mobile-nav";

export function Navbar() {
    const { user: { firstName = '', email = '' }, user } = useSelector(state => state.user);
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { theme, changeTheme } = useTheme();
    const [showSidebar, setShowSidebar] = useState(false);
    const [isLogoutVisible, setIsLogoutVisible] = useState(false);
    const isTabletOrMobile = useMediaQuery({ query: '(max-width: 1440px)' });

    const handleLogout = () => {
        localStorage.removeItem('retro-tube-token');
        dispatch(updateUser({}));
        navigate('/login');
    }

    useEffect(() => {
        return () => {
            setIsLogoutVisible(false);
        }
    }, [])

    const homeLink = (
        <NavLink to='/' className="navbar_brand nav_link align_center">
            Retro tube
        </NavLink>
    )

    return (
        <>
            <div className="nav_group">
                <nav className="nav_bar flex_row justify_spacebtw">
                    {isTabletOrMobile && (
                        <div className='mr-1' onClick={() => setShowSidebar(true)}>
                            <span className='material-icons pointer nav_hamburger'>
                                menu
                            </span>
                        </div>
                    )}
                    <div className="nav_options">
                        {isTabletOrMobile && homeLink}
                        {!isTabletOrMobile && (
                            <ul className="navbar_nav flex">
                                {homeLink}
                                <li className="nav_item active">
                                    <NavLink to='/explore' className="nav_link" state={{ categoryName: 'all' }}>
                                        Explore
                                    </NavLink>
                                </li>
                            </ul>
                        )}
                    </div>
                    {!isTabletOrMobile && (
                        <div className="flex align_center">
                            {email ?
                                <div className="relative_pos username_div">
                                    <h3 className="pointer username" onClick={() => setIsLogoutVisible((prevState) => prevState ? false : true)}>
                                        Hi, {firstName}
                                    </h3>
                                    {isLogoutVisible && (
                                        <button className="auth_btn logout_btn nav_link absolute_pos" onClick={handleLogout}>Logout</button>
                                    )}
                                </div>
                                :
                                <div className="nav_search flex align_center">
                                    <NavLink to='/signup' className="btn nav_link auth_btn">Sign up</NavLink>
                                    <NavLink to='/login' className="btn nav_link auth_btn">Log in</NavLink>
                                </div>
                            }
                            <label className="theme pointer" onClick={() => changeTheme()} title={theme}>
                                {theme === 'dark' ? <BsFillMoonFill /> : <BiSun />}
                            </label>
                        </div>
                    )}
                </nav>
            </div>
            {isTabletOrMobile && (
                <MobileNavigation
                    show={showSidebar}
                    close={() => setShowSidebar(false)}
                    user={user}
                    handleLogout={handleLogout}
                />
            )}
        </>
    );
}
